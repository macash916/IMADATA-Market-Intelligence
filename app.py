...
# The pre-built cache (1.6 MB) is committed to the repo.
# The raw municipality GeoJSON (154 MB) is NOT required on Streamlit Cloud.
_STATE_GEO_CACHE = os.path.join(BASE, "outputs", "state_mexico_cached.json")

@st.cache_data
def build_state_geojson():
    """Load pre-built state GeoJSON from cache, or build it from raw data if available."""
    if os.path.exists(_STATE_GEO_CACHE):
        with open(_STATE_GEO_CACHE, encoding="utf-8") as f:
            return json.load(f)

    # Fallback: build from raw municipality GeoJSON (local dev only)
    if not os.path.exists(GEO):
        st.error("State GeoJSON cache not found. Please commit outputs/state_mexico_cached.json to your repo.")
        st.stop()
